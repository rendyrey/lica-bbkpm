...
<?php
    ...
    <tr class="<?php echo $class_duplo ?>">
        ...
        <td style="border-right: 1px solid grey">
            ...
            @if (DateTime::createFromFormat('Y-m-d', $value->global_result) !== false)

            @php
            $result_date = date('d/m/Y', strtotime($value->global_result));
            @endphp
...
<script>
    ...
    function printTestGroup(groupId, transactionId) {
        $('#print-test-group-' + groupId).attr('href', baseUrl('printTestGroupPost/' + groupId + '/' + transactionId));
    }
