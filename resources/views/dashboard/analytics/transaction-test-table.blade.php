...
<?php
function uniqueCodeTest($value)
{
    $class = $value->unique_test_code ? '' : 'hidden';
    $icon = '<i data-toggle="tooltip" data-placement="top" title="' . $value->unique_test_code . '" class="bi bi-code-square " style="cursor:pointer" onClick="uniqueCodeTestModal(' . $value->id . ',' . $value->transaction_id . ',`' . $value->unique_test_code . '`)"></i>';
    $notif = ' <span class="note-notif ">' . $icon . '<span class="badge ' . $class . '">&nbsp;</span></span>';
...
<?php
function resultDate($value, $transactionTestId, $testId, $tabIndex)
{
    // if ($value->result_date) {
    //     $result_date = date('m/d/Y', strtotime($value->result_date));
    // } else {
    //     $result_date = '';
    // }
    ...
    // $input = '
    //     <input ' . $disabled . 'style="font-size: 10px" class="form-control form-control-sm" id="result-date-picker-' . $transactionTestId . '" placeholder="Pick Date" data-transaction-id="' . $value->transaction_id . '" data-transaction-test-id="' . $value->id . '" tabindex="' . $tabIndex . '" value="' . $result_date . '" onFocus="resultDate(' . $transactionTestId . ')">
    //     ';
    $input = '
            <input class="form-control form-control-solid form-control-sm input-date" id="result-date-picker" data-transaction-id="' . $value->transaction_id . '" data-transaction-test-id="' . $value->id . '" tabindex="' . $tabIndex . '" value="' . $value->result_date . '">
            ';
...
function descriptionType($value, $testId, $tabIndex)
{
    ...
    // check nomor sediaan FNAB Test
    $query = DB::table('tests')->where('id', $testId)->first();
    $test = $query;
    if ($test->name == 'Nomor Sediaan') {
        $checkNomorSediaan = 1;
    } else {
        $checkNomorSediaan = 0;
    }
    ...
    if ($value->mark_duplo == 1) {
        ...
        if ($checkNomorSediaan == 0) {
            ...
            // if result not null, then readonly
            if ($result == '' || $result == null) {
                $input = '
                    <textarea id="desc' . $value->id . '" data-existing="' . $result . '" id="nomor-sediaan" class="result-description form-control result-nomor-sediaan-' . $testId . '" tabindex="' . $tabIndex . '" data-transaction-id="' . $value->transaction_id . '" data-transaction-test-id="' . $value->id . '" onClick="clickNomorSediaanFNAB(' . $testId . ', ' . $value->transaction_id . ', ' . $value->id . ')">' . $result . '</textarea>
                ';
            } else {
                $input = '
                    <textarea id="desc' . $value->id . '" data-existing="' . $result . '" class="result-description form-control result-nomor-sediaan-' . $testId . '" tabindex="' . $tabIndex . '" data-transaction-id="' . $value->transaction_id . '" data-transaction-test-id="' . $value->id . '" readonly>' . $result . '</textarea>
                ';
            }
        }
    }
    ...
    <tr class="row-test-<?php echo $value->id ?>">
        ...
        <td style="border-right: 1px solid grey">
            <div class="d-flex justify-content-between">
                <span>{{$value->test->initial}}</span>
                <span>{!! testMemo($value) !!}</span>
            </div>
            @if ($value->flag_micro_head != null || $value->flag_micro_head != 0)
            <div class="d-flex justify-content-end">
                <span>{!! uniqueCodeTest($value) !!}</span>
            </div>
            @endif
        </td>
        <td style="border-right: 1px solid grey">
            ...
            @elseif($value->test->range_type == 'date')
            {!! resultDate($value, $value->id, $value->test_id, $key) !!}
            @endif
        </td>
...
<script>
    var datepickerInputDate;
    // const thisYear = new Date().getFullYear(); // returns the current year
    datepickerInputDate = $(".input-date").flatpickr({
        altInput: true,
        altFormat: 'd/m/Y',
        dateFormat: 'Y-m-d',
        static: true
    ...
    $(".input-date").on('change', function(e) {
        ...
        $.ajax({
            url: baseUrl('analytics/update-result-date/' + transactionTestId),
            type: 'put',
            data: {
                result_date: value
            },
            success: function(res) {
                toastr.success("Success update result date");
    ...
    function clickNomorSediaanFNAB(testId, transactionId, transactionTestId) {

        $.ajax({
            url: baseUrl('analytics/check-nomor-sediaan/' + transactionId + '/' + transactionTestId + '/' + testId),
            type: 'get',
            success: function(res) {
                toastr.success("Success update result text");
                $(".result-nomor-sediaan-" + testId).val(res.result_text);
            }
        });
    }
    ...
    function convertDate(date) {
        ...
        let theDate = year + '-' + ('0' + month).slice(-2) + '-' + ('0' + day).slice(-2);

        return theDate;
    }

    function resultDate(transactionTestId) {

        $("#result-date-picker-" + transactionTestId).daterangepicker({
        ...
        $("#result-date-picker-" + transactionTestId).on('change', function(e) {
        ...
        // $("#culture-date-picker-" + transactionTestId).on('focus', function(e) {
