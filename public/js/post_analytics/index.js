...
var onSelectTransaction = (selectedData) => {
  ...
  $("#created_time").data('finish-transaction-id', transactionId);

   // created time picker
   createdTimePicker.setDate(selectedData.created_time);
...
var checkMicrobiologyTest = function (transactionId) {
  $.ajax({
    url: baseUrl('post-analytics/check-microbiology-test/' + transactionId),
    type: 'GET',
    success: function(res) {
      ...
      if(res.confirm == 1){
        ...
        $(".serial-number-detail").html(res.finishTransactionData.no_urut);
        $(".register-number-detail").html(res.finishTransactionData.no_register);
        $(".identity-number-detail").html(res.finishTransactionData.no_identitas);
...
var createdTimePicker;
var createdTime = () => {
  const thisYear = new Date().getFullYear();  // returns the current year
  createdTimePicker = $(".datetimepicker-created-time").flatpickr({
    altInput: true,
    altFormat: 'd/m/Y ',
    dateFormat: 'Y-m-d ',
    static: true,
...
// Set Up Edit Created Time

var createdTimeOnChange = function(){
  $("#created_time").on('change', function(e) {

    const value = $(this).val();
    const transactionId = $(this).data('finish-transaction-id');

    // alert(transactionId)

    $.ajax({
        url: baseUrl('post-analytics/update-created-time'),
        type: 'put',
        data: {
            created_time: value,
        ...
        success: function(res) {
          PostAnalyticDatatable.refreshTable();
            toastr.success("Success update created time");
        }
    });
  });
}
...
$("#created_time").on('change', function(e) {
  ...
  $.ajax({
      url: baseUrl('analytics/update-created-time'),
      type: 'put',
      data: {
          created_time: value,
          transaction_id: transactionId,
