...
var onSelectTransaction = function (selectedData) {
  ...
  checkMicrobiologyTest(transactionId);
  ...
  // set transaction id for edit microbiology test
  $("#edit-microbiology-btn").data('transaction-id', transactionId);
  ...
  $("#btn-final-check-in").data('auto-nolab', autoNolab);
...
function drawSpecimenChange(transactionId, specimenId, event){
  if (!event.target.checked) {
    $('#undraw-modal').modal('show');

    var user = '';
    $('#user_list_undraw').on('change', function() {
      user = this.value;
    });

    $("#btn-final-undraw-all").on('click', function() {
      var undraw_reason = $('#undraw_reason').val();

      if (!$("#undraw-all-btn").data('auto-undraw')) {
        var undraw_reason = $('#undraw_reason').val();

        if(user != '' && undraw_reason != ''){
          $.ajax({
            ...
            data: {
              ...
              undraw_reason: undraw_reason,
              mode: "undraw"
            },
            success: function(res) {
              $('#undraw-modal').modal('hide');
              $('#undraw-reason').val('');
              $("#user_list_undraw").val('').trigger('change');
    ...
    $("#btn-final-draw-all").on('click', function() {
      var draw_reason = $('#sampling_reason').val();
...
var unDrawAllBtnComponent = function(){
    ...
    if (patientId != '' && patientId != null) {
      $.ajax({
        ...
        success: function(res) {
          ...
          $(".patient-form input[name='nik']").val(res.nik);
...
var checkMicrobiologyTest = function (transactionId) {
  $.ajax({
    url: baseUrl('pre-analytics/check-microbiology-test/' + transactionId),
    type: 'GET',
    success: function(res) {

      if(res.confirm == 1){
        $('#edit-microbiology-btn').show();
        // $('#check-in-btn').prop('disabled', true);
        // $('#draw-all-btn').prop('disabled', true);

        $('.microbiology-information').show();
        
        $(".serial-number-detail").html(res.transactionData.no_urut);
        $(".register-number-detail").html(res.transactionData.no_register);
        $(".identity-number-detail").html(res.transactionData.no_identitas);

      }else{
        $('#edit-microbiology-btn').hide();
        $('.microbiology-information').hide();
      }
    }
  })
}

var editMicroBiologiForm = function () {
  $("#edit-microbiology-btn").on('click', function() {
    const transactionId = $(this).data('transaction-id');
    $.ajax({
      url: baseUrl('pre-analytics/edit-microbiology-form/' + transactionId),
      type: 'GET',
      success: function(data) {
        $('#micro-confirm-modal').modal('show');
        $("#form-update-micro-confirm input[name='id']").val(transactionId);
        $("#form-update-micro-confirm input[name='serial_number']").val(data.no_urut);
        $("#form-update-micro-confirm input[name='register_number']").val(data.no_register);
        $("#form-update-micro-confirm input[name='identity_number']").val(data.no_identitas);
      }
    })
  });
}

var microBiologyFormValidate = function () {
  // Initialize
  $('#form-update-micro-confirm').validate({
    ignore: 'input[type=hidden], .select2-search__field, .ignore-this', // ignore hidden fields
    errorClass: 'fv-plugins-message-container invalid-feedback',
    successClass: 'validation-valid-label',
    validClass: 'validation-valid-label',
...
var microBiologyFormSubmit = function () {
  const transactionId = $("#form-update-micro-confirm input[name='id']").val();

  $("#form-update-micro-confirm").on('submit', function(e) {
    
    e.preventDefault();
    if ($(this).valid()) {
        let theForm = $("#form-update-micro-confirm");
        let formData = $("#form-update-micro-confirm").serialize();

        $.ajax({
          url: baseUrl('pre-analytics/update-microbiolgy-number'),
          data: formData,
          type: 'put',
          success: function(data) {
            toastr.success(data.message);
            $('#micro-confirm-modal').modal('hide');
            $('#check-in-btn').prop('disabled', false);

            // data.microbiologyData.forEach(function(json) {
            //   $(".serial-number-detail").html(json.no_urut);
            //   $(".register-number-detail").html(json.no_register);
            //   $(".identity-number-detail").html(json.no_identitas);
            // });
            $.each(data.microbiologyData,function(index, json){
              $(".serial-number-detail").html(json.no_urut);
              $(".register-number-detail").html(json.no_register);
              $(".identity-number-detail").html(json.no_identitas);
            });
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  birthdate();
  drawAllBtnComponent();
  unDrawAllBtnComponent();
  syncBtn();
  checkInBtn();
  editMicroBiologiForm();
  microBiologyFormValidate();
  microBiologyFormSubmit();
  // checkInProcess();
  transactionNote();
  newPatientMedrec();
  editPatientDetails();
