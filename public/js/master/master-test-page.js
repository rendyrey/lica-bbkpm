...
var setValueModalEditForm = function(data)
{
  ...
  if ($("#modal_form_horizontal select[name='range_type']").val() == 'description' || $("#modal_form_horizontal select[name='range_type']").val() == 'label') {
    ...
    if(data.flag_micro_head != null){
        $('#micro-head-yes-edit').prop('checked', true);
    }else{
        $('#micro-head-none-edit').prop('checked', true);
    }

    var selectedVal = "";
    $('input[type=radio][name=radiosMicroHeadEdit]').change(function() {
        selectedVal = this.value;

        $.ajax({
        url: baseUrl('master/update-micro-head'),
        data: {
            test_id: data.id,
            microHeadValue: selectedVal,
        },
        type: 'PUT',
        success: function(data) {
            // toastr.success('Spuit options has been updated');
        }
        });
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    $(".range-type").on('change', function (e) {
        if($(this).val() == 'number') {
            ...
            $("#flag-micro-head").toggle(false);
        } else {
            if($(this).val() == 'label'){
                $("#flag-micro-head").toggle(true);
            }else{
                $("#flag-micro-head").toggle(false);
            }
    ...
    $(".range-type-edit").on('change', function (e) {
        if($(this).val() == 'number') {
            ...
            $("#flag-micro-head-edit").toggle(false);
        } else {
            if($(this).val() == 'label'){
                $("#flag-micro-head-edit").toggle(true);
            }else{
                $("#flag-micro-head-edit").toggle(false);
            }
