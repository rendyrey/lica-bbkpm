                ...
                if ($transaction) {
                    ...
                    foreach ($result['results'] as $test) {
                        ...
                        if ($interfacing_test != null) {
                            ...
                            // $ranges = DB::table('ranges')->where('test_id', $test_id)->get();
                            // if($ranges != null){
                            //     if(count($ranges) > 1){
                            //         foreach($ranges as $range){
                            //             if(($birthday >= $range->min_age)&&($birthday <= $range->max_age)){
                            //                 $range_ref = $range;
                            //             }
                            //         }
                            //     }else{
                            ...
                            if ($range) {
                                ...
                                if ($test->range_type == 'number') {
                                    ...
                                    // on development
                                    // /*Ambil master result berdasrkan id test dan nilai*/
                                    if($test->name == 'Nitrit') {
                                        $nValue = 3;
                                        if($result_value == 'pos'){
                                            $nValue = 1;
                                        ...
                                        $master_result = DB::table('master_results')->where('test_id', $test->id)
                                            ->where('min_range', '<=', $nValue)
                                            ->where('max_range', '>=', $nValue)
                                            ->first();
                                    }else{
                                        $master_result = DB::table('master_results')->where('test_id', $test->id)
                                    ...
                                    if ($trans_tests) {
                                        ...
                                        $data_test['result_text'] = $result_value;
                                        $data_test['result_status'] = $result_status;
                                        $trans_tests = true;
                                        // $$trans_tests = DB::table('transaction_tests')->insert($data_test);
                                    }
