...
class AnalyticController extends Controller
{
    ...
    /**
     * Check Microbiology confirmation, if there is microbilogy test in transaction
     */
    public function checkMicrobiology($transactionId)
    {
        $query_transaction = DB::table('transactions')->select('no_urut', 'no_register', 'no_identitas')
            ->where('id', $transactionId);
        $transactionData = $query_transaction->first();

        $query_test = DB::table('transaction_tests')->select('transaction_tests.id as transaction_test_id', 'transaction_tests.package_id', 'tests.name as test_name', 'specimens.name as specimen_name')
            ->leftJoin('tests', 'tests.id', 'transaction_tests.test_id')
            ->leftJoin('specimens', 'specimens.id', 'tests.specimen_id')
            ->where('transaction_tests.transaction_id', $transactionId)
            ->where('specimens.name', 'Lain-lain')
            ->groupBy('package_id');
        // $data = $query->get();
        $test_confirm = $query_test->first();
        ...
        return response()->json(['confirm' => $confirm, 'transactionData' => $transactionData]);
    }

    /**
     * Check Validated Test for send half data purposes
     */
    public function checkValidatedTest($transactionId)
    {
        $query_test = DB::table('transaction_tests')->select('transaction_tests.*', 'tests.name as test_name')
            ->leftJoin('tests', 'tests.id', 'transaction_tests.test_id')
            ->where('transaction_tests.transaction_id', $transactionId)
            ->where('transaction_tests.validate', 1);

        $testData = $query_test->get();
        $countData = $testData->count();
        ...
        return response()->json($existData);
    }
    ...
    {
        $model = \App\TransactionTest::selectRaw('transaction_tests.*, transaction_tests.id as tt_id, tests.flag_micro_head, results.result as res_label, packages.name as package_name')
            ->leftJoin('tests', 'tests.id', 'transaction_tests.test_id')
    ...
    public function checkFormula($transactionId, $transactionTestId, $testId)
    {
        ...
        return response()->json(['final_result' => $result_final, 'label' => $result_status]);
    }

    public function checkNomorSediaan($transactionId, $transactionTestId, $testId)
    {

        $query_transaction_tests = DB::table('transaction_tests')
            ->select('id as transaction_test_id', 'transaction_id', DB::raw('MAX(result_text) AS result'))
            ->where('test_id', $testId);
        $data = $query_transaction_tests->first();
        ...
        if ($data->result == '' || $data->result == NULL) {
            $prefixText = $firstText . $year . '346';
        } else {
            $substr = substr($currentResult, 3);
            $substr = $substr + 1;
            $endText = sprintf("%03s", $substr);
            $prefixText = $firstText . $year . $endText;
        }

        $updateData = array(
            'result_text' => $prefixText,
            'result_input_time' => date('Y-m-d H:i:s')
        );

        DB::table('transaction_tests')->where('id', $transactionTestId)->update($updateData);

        return response()->json(['message' => 'Success update result text', 'result_text' => $prefixText]);
    }
    ...
    public function updateResultNumber($transactionTestId, Request $request)
    {
        try {
            ...
            $transactionTest->result_input_time = Carbon::now()->toDateTimeString();
    ...
    public function updateResultDate($transactionTestId, Request $request)
    {
        try {
            ...
            $transactionTest->result_date = $date;
            $transactionTest->result_input_time = Carbon::now()->toDateTimeString();
    ...
    public function verifyAll($transactionId)
    {
        try {
            ...
            foreach ($transactionTests as $value) {
                if ($value->result_number || $value->result_number != "" || $value->result_label || $value->result_text || $value->result_date) {
                    $value->verify = true;
                ...
                if($value->result_text == "" && $value->test_id == "514"){
                    $value->result_text = "Hasil Terlampir";
                    $value->verify = true;
                    $value->verify_by = $user->id;
                    $value->verify_time = $now;
                    $value->save();
    ...
    public function verifyTest(Request $request, $transactionTestId)
    {
        try {
            ...
            if($transactionTest->result_text == "" && $transactionTest->test_id == "514"){
                $transactionTest->result_text = "Hasil Terlampir";
                $transactionTest->verify = $request->value;
                $transactionTest->verify_by = $user->id;
                $transactionTest->verify_time = Carbon::now();
            }
            // if ($transactionTest->result_number == "") {
            //     throw new \Exception("Unable to verify because result has not been set");
            // }
    ...
    public function validateAll($transactionId)
    {
        try {
            ...
            foreach ($transactionTests as $value) {
                if ($value->verify == 1 && $value->validate_time == null) {
                    $value->validate = true;
    ...
    public function updateUniqueTestCode(Request $request)
    {
        try {
            $transactionTestId = $request->transaction_test_id;
            $unique_test_code = $request->unique_test_code;

            $transactionTest = \App\TransactionTest::where('id', $transactionTestId)->first();

            $transactionTest->unique_test_code = $unique_test_code;
            $transactionTest->save();
    ...
    public function sendDataAction($transactionId)
    {
        $transactionData = DB::table('transactions')->where('id', $transactionId)->first();
        $no_order = $transactionData->no_order;

        if ($no_order != NULL || $no_order != '') {
            app(\App\Http\Controllers\ApiController::class)->sendHalfData($no_order);
        }
    ...
    public function goToPostAnalytics($transactionId)
    {
        try {
            ...
            $data_insert_transactions = [
                ...
                'no_urut' => $transactions->no_urut,
                'no_register' => $transactions->no_register,
                'no_identitas' => $transactions->no_identitas,
            ...
            foreach ($transactions_test as $key => $value) {
                ...
                $dt = [
                    ...
                    "result_input_time" => $value->result_input_time,
                    "verify" => $value->verify,
                ...
                if ($value->result_date) {
                    $dt['global_result'] = $value->result_date;
                }
                // else {
                //     $dt['global_result'] = "-";
                // }
    ...
    public function updateVerificatorValidator(Request $request)
    {
        $transaction_id = $request->input("transaction_id");
        $verificator_id = $request->input("verificator_id");
        $validator_id = Auth::user()->id;

        // echo 'ID Finish Transaksi: ' . $finish_transaction_id . '<br>';
        // echo 'ID Verifikator: ' . $verificator_id . '<br>';
        // echo 'Username Login: ' . Auth::user()->name . '<br>';

        // UPDATE id validator/verificator in transactions table
        DB::table('transactions')
            ->where('id', $transaction_id)
            ->update(
                [
                    'verficator_id' => $verificator_id,
                    'validator_id' => $validator_id
                ]
            );

        echo 'Berhasil update Verificator dan Validator' . '<br>';
        echo 'ID Transaksi : ' . $transaction_id . '<br>';
        echo 'ID Verificator : ' . $verificator_id . '<br>';
        echo 'ID Validator : ' . $validator_id . '<br>';
    }
    ...
    public function updateCreatedTime(Request $request)
    {
        $updateData = array('created_time' => $request->created_time);
        
        $query = DB::table('transactions')->where('id', $request->transaction_id)->update($updateData);

   

        return response()->json('update created time success');
    }
}