...
class PreAnalyticController extends Controller
{
    ...
    public function datatableTest($roomClass)
    {
        ...
        // $model = \App\TestPreAnalyticsView::selectRaw("*")->where('class', $roomClass);
    ...
    public function datatableEditTest($roomClass, $transactionId)
    {
        ...
        foreach ($transactionTests as $testId) {
            ...
            }else if ($testId['type'] == 'grand-package') {
                $uniqueIds[] = 'gp' . $roomClass . '-' . $testId['grand_package_id'];
                $uniqueZeroClassIds[] = 'gp0-' . $testId['grand_package_id'];
            }
        }
        ...
        // })->where('class', $roomClass)->whereNotIn('unique_id', $uniqueIds)->whereNotIn('unique_id', $uniqueZeroClassIds)->groupBy('unique_id');
     ...
     * Check Microbiology confirmation, if there is microbilogy test in transaction
     */
    public function editMicrobiologyForm($transactionId)
    {
        $query = DB::table('transactions')->select('no_urut', 'no_register', 'no_identitas')
            ->where('id', $transactionId);
        $data = $query->first();
     ...
     * 
     */
    public function updateMicrobiologyNumber(Request $request)
    {
        try {
            ...
            $array_response = array('no_urut' => $serial_number, 'no_register' => $register_number, 'no_identitas' => $identity_number);

            \App\Transaction::where('id', $request->id)
                ->update([
                    'no_urut' => $serial_number,
                    'no_register' => $register_number,
                    'no_identitas' => $identity_number
                ]);
            return response()->json(['message' => 'Success update microbiology numbers', 'microbiologyData' => $array_response]);
        } catch (\Exception $e) {
            return response()->json(['message' => $e->getMessage()], 400);
        }
    }
    ...
    public function updateDraw(Request $request)
    {
        try {
            ...
            if ($request->mode == "draw") {
                $data_update['draw'] = 1;
                $data_update['draw_by'] = $request->user;
                $data_update['draw_memo'] = $request->reason;
                ...
                $data_update['undraw_by'] = $request->user;
                $data_update['undraw_memo'] = $request->undraw_reason;
    ...
    public function drawAll($value, Request $request)
    {
        ...
        try {
            ...
            \App\TransactionTest::where('transaction_id', $request->transaction_id)
                ...
                ->update([
                    'draw' => $value,
                    'draw_by' => ($value) ? $request->user : null,
                    'draw_time' => ($value) ? Carbon::now() : null,
                    'draw_memo' => ($value) ? $request->reason : null,
                    'undraw_by' => (!$value) ? $request->user : null,
                    'undraw_memo' => (!$value) ? $request->undraw_reason : null
                ]);
