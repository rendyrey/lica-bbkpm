...
class PrintController extends Controller
{
    ...
    public function hasilTest($id)
    {
        ...
        $query_tests = DB::table('finish_transaction_tests')
            ...
            ->where('is_print', '=', 1)
            ->where('group_name', '!=', 'FNAB')
            ->where('group_name', '!=', 'PLEURA')
            ->orderBy('sequence', 'asc')
            ->orderBy('sub_group', 'asc');
        ...
        //     ->select('*')
        //     ->where('finish_transaction_id', '=', $id)->orderBy('id', 'asc')->first();
        // if ($log_print) {
        ...
        $print_time_query = DB::table('finish_transaction_tests')
            ->select(DB::raw('MAX(validate_time) AS validate_time'))
            ->where('finish_transaction_id', '=', $id)->first();
        if ($print_time_query) {
    ...
    public function hasilTestEnglish($id)
    {
        // $id = 31;
        // dd($id);
        // $testArr = json_decode($testlist);
        ...
        return view('prints.hasil-test-english', $data);
    }

    public function hasilTestGroup($groupId, $transactionId)
    {
        $judul = "Print Result";
        $data['title'] = $judul;
        ...
        ->select('finish_transactions.id')
        ->where('transaction_id', $transactionId)
        ->first();

        // dd($finish_transaction_id);

        // Group Data
        $query_group = DB::table('groups')->where('id', $groupId);
        $groupData = $query_group->first();

        if ($groupData->print_format_post == 'hasil-test-group') {
            $transaction = DB::table('finish_transactions')
                ->select('finish_transactions.*')
                ->where('finish_transactions.transaction_id', '=', $transactionId)->first();
            ...
            // $log_print = DB::table('log_print')
            //     ->select('*')
            //     ->where('transaction_id', '=', $transactionId)->orderBy('id', 'asc')->first();
            // if ($log_print) {
            ...
            $born = Carbon::createFromFormat('Y-m-d', $transaction->patient_birthdate);
            $age = $born->diff(Carbon::now())->format('%y Thn %m Bln %d Hr');
            $group_draw_time_query = DB::table('finish_transaction_tests')
                ->select(DB::raw('MIN(draw_time) AS draw_time'))
                ->where('transaction_id', '=', $transactionId)
            ...
            // }
              ...
              // print time       
        $print_time_query = DB::table('finish_transactions')
        ->select('finish_transactions.*')
        ->where('finish_transactions.transaction_id', '=', $transactionId)->first();
    if ($print_time_query) {
        $print_time = $print_time_query->created_at;
    } else {
        $print_time = Carbon::now();
    }
              // di jawab tanggal       
        $jawab_tanggal_query = DB::table('transactions')
        ->select('transactions.result_date')
        ->where('transactions.id', '=', $transactionId)->first();
    if ($jawab_tanggal_query) {
        $jawab_tanggal = $jawab_tanggal_query->result_date;
    } else {
        $jawab_tanggal = Carbon::now();
    }
            ...
            $data = [
                ...
                "transaction" => $transaction,
                "age" => $age,
                "jawab_tanggal" => $jawab_tanggal
            ];

            return view('prints.hasil-test-group', $data);
        } else if ($groupData->print_format_post == 'hasil-test-patologi') {
            $transaction = DB::table('finish_transactions')
                ->select('finish_transactions.*')
                ->where('finish_transactions.transaction_id', '=', $transactionId)->first();

            $transaction_test = DB::table('finish_transaction_tests')
                ->select('finish_transaction_tests.*')
                ->where('finish_transaction_tests.finish_transaction_id', '=', $transaction->id)->first();
            ...
            $query_tests = DB::table('finish_transaction_tests')
                ...
                ->where('is_print', '=', 1)
                ->where('group_id', '=', $groupId)
                ->where('test_name', '!=', 'Nomor Sediaan')
                ->where('test_name', '!=', 'Dijawab Tanggal')
                ->orderBy('sequence', 'asc')
                ->orderBy('sub_group', 'asc');
            ...
            $query_custom = DB::table('finish_transaction_tests')
                ->where('transaction_id', '=', $transactionId)
                ->where('is_print', '=', 1)
                ->where('group_id', '=', $groupId)
                ->orderBy('sequence', 'asc')
            ...
            $customData = $query_custom->get();
            foreach ($customData as $custom_data) {
                if ($custom_data->test_name == 'Nomor Sediaan') {
                    $nomor_sediaan = $custom_data->result_text;
                } else if ($custom_data->test_name == 'Dijawab Tanggal') {
                    $dijawab_tanggal = $custom_data->result_text;
                }
            }
            ...
            $data = [
                ...
                "nomor_sediaan" => $nomor_sediaan,
                "dijawab_tanggal" => $dijawab_tanggal,
                "jawab_tanggal" => $jawab_tanggal
            ];

            return view('prints.hasil-test-patologi', $data);
        }
    }
    ...
    public function printAnalytic($id, $group_id = null)
    {
        ...
        $query_transaction_tests = DB::table('transactions')
            ...
            ->where('groups.name', '!=', 'FNAB')
            ->where('groups.name', '!=', 'PLEURA')
            ->orderBy('sequence', 'asc')
            ->orderBy('sub_group', 'asc');
        ...
        // last draw time
        $last_draw_time_query = DB::table('transaction_tests')
            ->select(DB::raw('MAX(draw_time) AS draw_time'))
            ->where('transaction_id', '=', $id)->first();
        if ($last_draw_time_query) {
            $last_draw_time = $last_draw_time_query->draw_time;
    ...
    public function printAnalyticEnglish($id, $group_id = null)
    {
        // dd($id);
        $judul = "Print Result";
        $data['title'] = $judul;
        ...
        return view('prints.group-result-analytic-english', $data);
    }
    ...
    public function printTestGroup($groupId, $id)
    {
        ...
        // print-test-group
        // ==========================================
        if ($groupData->print_format_analytic == 'print-test-group') {
            $query_transaction = DB::table('transactions')
                ->select(
                    'transactions.id as transaction_id',
                    'transactions.checkin_time',
        ...
        // print-test-patologi
        // ==========================================
        else if ($groupData->print_format_analytic == 'print-test-patologi') {
            $query_transaction = DB::table('transactions')
                ->select(
                    'transactions.id as transaction_id',
                    'transactions.checkin_time',
            ...
            // get nomor sediaan & tanggal dijawab
            $query_custom = DB::table('transactions')
                ->select(
                    'transactions.id as transaction_id',
                    'tests.name as test_name',
                    'tests.sub_group as sub_group',
            ...
            foreach ($customData as $custom_data) {
                if ($custom_data->test_name == 'Nomor Sediaan') {
                    ...
                    $dijawab_tanggal = $custom_data->result_date;
                }
            }
            ...
            return view('prints.print-test-patologi', $data);
        }
    }
    ...
    public function showBarcode($id)
    {
        ...
        foreach ($data_transactions as $transaction) {
            ...
            if (strlen($transaction->patient_name) <= 21) {
                $patient_name = $transaction->patient_name;
            } else {
                $patient_name = substr($transaction->patient_name, 0, 21);
            }
            ...
            $dataToWrite = $dataToWrite . "^FO0,15^FD" . $patient_name . "\&^FS";
