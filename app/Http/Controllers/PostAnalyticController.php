...
class PostAnalyticController extends Controller
{
     ...
     */
    public function checkMicrobiology($finishTransactionId)
    {

        $query_transaction = DB::table('finish_transactions')->select('no_urut', 'no_register', 'no_identitas')
            ->where('id', $finishTransactionId);
        $finishTransactionData = $query_transaction->first();

        $query_test = DB::table('finish_transaction_tests')->select('id as finish_transaction_test_id', 'package_id', 'test_name', 'specimen_name')
            ->where('finish_transaction_id', $finishTransactionId)
            ->where('specimen_name', 'Lain-lain')
            ->groupBy('package_id');
        // $data = $query->get();
        ...
        return response()->json(['confirm' => $confirm, 'finishTransactionData' => $finishTransactionData]);
    }
    ...
    public function updateCreatedTime(Request $request)
    {
        ...
        $query = DB::table('finish_transactions')->where('id', $request->transaction_id)->update($updateData);
